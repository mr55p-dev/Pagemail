...
func TestSendMailToUser(t *testing.T) {
	...
	dbReader.numUsers = 1
	assert := assert.New(t)
	err := SendUserDigest(context.TODO(), &testUser, dbReader, mailSender, now)
	assert.NoError(err)
	assert.Len(mailSender.mail, 1)
	assert.Equal(mailSender.mail[0].To, "user@mail.com")
}
...
func TestDoMailJob(t *testing.T) {
	...
	dbReader.numUsers = 2
	assert := assert.New(t)
	err := DigestJob(context.TODO(), dbReader, mailSender, now)
	assert.NoError(err)

...
install-nginx:
	...
	sudo certbot certonly --nginx --dry-run -d "v2.pagemail.io,www.pagemail.io,pagemail.io,staging.pagemail.io" --expand --non-interactive
...
install-stage-frontend:
	...
	cp $(BASE_DIR)/client/dist/* $(STAGE_WEB_TARGET)/
...
install-prod-frontend:
	...
	cp $(BASE_DIR)/client/dist/* $(PROD_WEB_TARGET)/
...
pre-install:
	# If $(BASE_DIR) exists delete everything in it
	if [ -d $(BASE_DIR) ]; then rm -rf $(BASE_DIR)/*; fi
	mkdir -p $(BASE_DIR)/client/dist
	mkdir -p $(BASE_DIR)/server/dist
